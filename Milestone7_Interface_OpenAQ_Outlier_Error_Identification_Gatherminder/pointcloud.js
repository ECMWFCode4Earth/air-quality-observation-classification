...
function generatePointCloudGeometry( color, width, length ){
	...
	for( var i = 0; i < width; i++ ) {
		...
		for( var j = 0; j < length; j++ ) {
			...
			var intensity = 1; //( y + 0.1 ) * 5; // number between 0 and 1
	...
	**/
	...
	for( var i = 0; i < seriesCount; i++ ) {
		...
		for( var j = 0; j < seriesLength; j++ ) {
			...
			var u = i / seriesCount;
			var v = j / seriesLength;
			var x = u - 0.5;
			var y = ( Math.cos( u * Math.PI * 8 ) + Math.sin( v * Math.PI * 8 ) ) / 20;
			var z = v - 0.5;

			positions[ 3 * k ] = i/seriesCount; // x coordinate
			positions[ 3 * k + 1 ] = j/seriesLength; // y coordinate
			positions[ 3 * k + 2 ] = data[i][j]/50; // z coordinate
			// should this be data[labels[i]][j]?
	...
	**/
	...
	for( var i = 0; i < seriesCount; i++ ) {
		...
		for( var j = 0; j < seriesLength; j++ ) {
			...
			var v = new THREE.Vector3( i,j,data[i][j] );
			// should this be data[labels[i]][j]?
...
function init() {
	...
	container = document.getElementById( 'pointcloud_container' );
