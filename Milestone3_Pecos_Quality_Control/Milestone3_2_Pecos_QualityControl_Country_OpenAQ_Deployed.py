...
def Milestone3_Pecos_Quality_Control_EveryStation(OpenAQ_Stations, parameter, Completed_QC_Processes, VisualAnalytics_Complete, OpenAQNearestHighway):
 ...
 #  print(OpenAQ_Stations)
   ...
   #   print(OpenAQStation) 
      ...
      if(iterationamount >= Completed_QC_Processes):
        ...
        if(len(OpenAQStation_Dataset) > 0): 

          Milestone2_OpenAQ_Dataset_VisualAnalytics(VisualAnalytics_Complete, OpenAQStation, OpenAQStation_Dataset)
          ...
          Milestone4_OpenAQ_Get_NearestHighway(OpenAQStation, OpenAQStation_Dataset, OpenAQNearestHighway)
...
def Milestone2_OpenAQ_Dataset_VisualAnalytics_Histogram(df4, parameter, OpenAQStation, title="", xlabel='Value', ylabel='Amount of Measurements', dpi=100):
...
# Step Create a Histogram of the OpenAQ Dataset for parameter
 ...
 # Query the data to only get positive values and resample to hourly
  # _df = df4.query("value >= 0.0") # .resample('1h').mean()

  # _df.value.plot(ax=ax, label=OpenAQStation)
...
def Milestone2_OpenAQ_Dataset_VisualAnalytics(VisualAnalytics_Complete, OpenAQStation, OpenAQStations):
    
    
   if(VisualAnalytics_Complete == 1): 
    
     OpenAQDataset_VisualAnalytics = "OpenAQ Dataset Station " + CountryCode +" " + OpenAQStation + " " + parameter + " Time Schedule " + str(dt_begin) + " to " + str(dt_end)

     Milestone2_OpenAQ_Dataset_VisualAnalytics_Histogram(OpenAQStations, parameter, OpenAQStation, title=OpenAQDataset_VisualAnalytics, xlabel='Value', ylabel='Amount of Measurements', dpi=100) 
...
def Milestone4_OpenAQ_Get_NearestHighway(OpenAQunique, df4, OpenAQNearestHighway):
   ...
   #   print(OpenAQAPIdatasetunique)
    ...
    OpenAQAPIdatasetuniqueDataset.append(df4['value'].describe()['mean'])
...
def Test_Milestone3_Import_OpenAQ_Dataset_StationOpenAQ(OpenAQStation):
    
    
   res = api.latest(location=OpenAQStation, df=True)
