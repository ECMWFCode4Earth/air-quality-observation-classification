...
## 1 Choose an upper and lower bound of values and identify value over to be outliers by searching openAQ dataset

Quality Control Defininition: "check if data is within expected bounds. i.e. below Upper and lower Bound. Range tests are very flexible. The test can be used to check for expected range on the raw data or using modified data"

## 2 Choose an increment on measurements that is acceptable for both increasing and decreasing values 
...
## 3 Find how many stations don’t measure every 15 minutes through a day and when these are

Quality Control Defininition: "check the time index for missing, duplicate, and non-monotonic indexes"

## 4 Find stagnant measurements that don't change over a chosen amount 
...
## 5 Find outliers due to a test on a selection of measurements. 
...
## 6 Find Corrupt Dataset measurement

Quality Control Definition: "selected measurements"
